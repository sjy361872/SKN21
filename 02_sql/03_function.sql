...
select char_length('aaabbbcc');
select char_length(trim('   aa   '))   "처리결과";
...
select emp_id,
	   ...
	   end "job" -- 출력값에 column명을 작성 ->원래 ( 그 형의 ) 값을 출력
from   emp
where  job in ('AD_PRES', 'FI_ACCOUNT', 'PU_CLEK', 'IT_PROG');
